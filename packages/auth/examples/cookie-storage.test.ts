...
import { app, secret, clientId, issuerUrl, encryptToken, decryptToken } from './cookie-storage'
...
describe('Cookie storage', async () => {
  ...
  test.only('Authorization Code Flow', async () => {
    ...
    expect(authorizationUrl.searchParams.get('redirect_uri')).toStrictEqual('/callback')
    ...
    const accessTokenCookie = await getSetCookie(response, 'access_token', decryptToken)
    const idTokenCookie = await getSetCookie(response, 'id_token', decryptToken)
    const refreshTokenCookie = await getSetCookie(response, 'refresh_token', decryptToken)

    response = await app.request('/session', {
      headers: {
        cookie: [
          `${sessionCookie?.name}=${sessionCookie?.value}`,
          `${accessTokenCookie?.name}=${await encryptToken(tokens.access_token ?? '')}`,
          `${idTokenCookie?.name}=${await encryptToken(signedIdToken ?? '')}`,
          `${refreshTokenCookie?.name}=${await encryptToken(tokens.refresh_token ?? '')}`,
        ].join('; '),
      },
    })
    ...
    const response = await app.request('/session', {
      headers: {
        cookie: [
          ...
          `id_token=${await encryptToken(tokens.id_token)}`,
  ...
  it('deletes session data on signout', async () => {
    ...
    const sessionCookie = await getSetCookie(response, 'sid', decrypt)
    const accessToken = await getSetCookie(response, 'access_token', decryptToken)
    const idToken = await getSetCookie(response, 'id_token', decryptToken)
    const refreshToken = await getSetCookie(response, 'refresh_token', decryptToken)
