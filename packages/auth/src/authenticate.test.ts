...
describe('Authenticate middleware', () => {
  ...
  const app = new Hono()
    ...
    .get('/session', authenticate({ redirectUri: '/callback' }), async (c) => {
...
describe('Authenticate middleware refresh token', () => {
  const customFetch = vi.fn<client.CustomFetch>(authResponse())
      ...
      authenticate({
        ...
        async refreshSession(c, expired) {
          if (expired.tokens?.refresh_token) {
            const { tokens } = await c.var.auth.handleRefresh(expired.tokens.refresh_token)
